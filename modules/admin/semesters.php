...
$adminSection = 'semesters';
$error = '';
$success = '';

// Create table if not exists
$conn->query("CREATE TABLE IF NOT EXISTS chmsu_semesters (
...
if(isset($_POST['action'])) {
    if($_POST['action'] == 'add') {
        $semester_name = sanitize($_POST['semester_name']);
        $semester_code = sanitize($_POST['semester_code']);
        
        // Check if semester already exists
        $check = $conn->query("SELECT * FROM chmsu_semesters WHERE semester_name='$semester_name' OR semester_code='$semester_code'");
        if($check && $check->num_rows > 0) {
            $error = "Semester already exists!";
        } else {
            $conn->query("INSERT INTO chmsu_semesters (semester_name, semester_code) VALUES ('$semester_name', '$semester_code')");
            logActivity($conn, $admin, 'admin', "Added semester: $semester_name");
            $success = "Semester added successfully!";
        }
    }
    elseif($_POST['action'] == 'set_current') {
        $id = intval($_POST['id']);
        $conn->query("UPDATE chmsu_semesters SET is_current = 0");
        $conn->query("UPDATE chmsu_semesters SET is_current = 1 WHERE id = $id");
        logActivity($conn, $admin, 'admin', "Set current semester");
...
<head>
    ...
    <title>CHMSU E-Clearance System - Manage Semesters</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body.dark-mode .badge-current { background: #1a7a3a; }
        body.dark-mode .badge-code { background: #333; color: #f1c40f; }
...
<div class="dashboard-wrapper">
    ...
    <div class="main-content">
        ...
        <div class="content-card">
            <div class="content-card-header">Add New Semester</div>
                ...
                <form method="POST">
                    ...
                    <div class="filter-row">
                        <div class="form-group" style="flex:1;">
                            ...
                            <input type="text" name="semester_name" placeholder="First Semester" required>
                        </div>
                        <div class="form-group" style="flex:0 0 120px;">
                            <label>Semester Code</label>
                            <input type="text" name="semester_code" placeholder="1st Sem" required>
                        </div>
        ...
        <div class="content-card">
            <div class="content-card-header">Semester List</div>
            <div class="content-card-body">
                <ul class="item-list">
                    <?php 
                    $semesters = $conn->query("SELECT * FROM chmsu_semesters ORDER BY id DESC");
                    ...
                    <li>
                        <span>
                            ...
                            <span class="badge-code"><?php echo htmlspecialchars($sem['semester_code']); ?></span>
                            <?php if($sem['is_current']): ?>
                                <span class="badge-current"><i class="fas fa-check-circle"></i> Current</span>
                            <?php endif; ?>
                        </span>
                        <span>
                            <?php if(!$sem['is_current']): ?>
                            ...
                            <?php endif; ?>
                            <button type="button" class="btn btn-danger btn-sm" onclick="openOTPModal('semesters', <?php echo $sem['id']; ?>, '<?php echo addslashes($sem['semester_name'] . ' (' . $sem['semester_code'] . ')'); ?>')">
                    ...
                    ?>
                    <li style="text-align:center;">No semesters found. Add your first semester above.</li>
                    <?php endif; ?>
                </ul>
            </div>
        </div>
